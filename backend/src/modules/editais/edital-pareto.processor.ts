...
export class EditalParetoProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    const paretoAnalysis = await this.langChainService.analyzeEditalPareto(job.data.pdfText, job.data.title);
    return await this.supabaseService.addEdital(job.data.title, job.data.userId, paretoAnalysis);
  }
}
