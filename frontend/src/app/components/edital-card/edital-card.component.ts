...
@Component({
  ...
  template: `
    <div class="bg-gradient-to-br from-[#f8f9ff] via-[#fbfbfe] to-[#edf2fe] dark:bg-gradient-to-br dark:from-[#13162d] dark:via-[#161a37] dark:to-[#1a1b3f] rounded-3xl p-4 sm:p-6 flex flex-col justify-between border-2 border-indigo-200/90 dark:border-indigo-500/40 hover:border-indigo-400 dark:hover:border-indigo-400/80 shadow-md shadow-indigo-500/5 hover:shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1 transition-all duration-300 h-full relative overflow-hidden group text-[var(--on-surface)]">
      ...
      <div class="relative z-10 pt-1">
        ...
        <div class="flex items-center justify-between gap-2 mb-3.5 flex-wrap">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span class="bg-[#5d3bf6]/10 text-[#5d3bf6] dark:text-[#a78bfa] text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 border border-[#5d3bf6]/20">
              <span class="material-symbols-outlined !text-[15px]">stars</span>
              {{ edital.pareto_data?.high_priority_subjects || 2 }} Matérias Vitais (20%)
            </span>
            <span *ngIf="edital.concurso" class="bg-[#5d3bf6]/10 text-[#5d3bf6] dark:text-[#a78bfa] text-xs font-extrabold px-3 py-1 rounded-full border border-[#5d3bf6]/20">
              {{ edital.concurso }}
            </span>
            <span *ngIf="getEditalBancaName(edital)" class="inline-flex items-center gap-1.5 bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/20 text-xs font-bold px-2.5 py-1 rounded-full shadow-2xs">
              <span *ngIf="getEditalBanca(edital)" class="h-3.5 w-6 flex items-center justify-center bg-white rounded px-0.5 shadow-2xs border border-[var(--outline-variant)]/60">
          ...
          <span class="text-xs font-bold text-[var(--on-surface-variant)]">{{ edital.created_at | date:'dd/MM/yyyy' }}</span>
        </div>

        <!-- Title & Cargo -->
        <h3 class="text-lg sm:text-xl font-black text-[var(--on-surface)] mb-1.5 leading-snug flex items-center gap-2">
          <span class="material-symbols-outlined !text-[20px] text-[#5d3bf6] shrink-0">description</span>
        ...
        <p class="text-xs sm:text-[13px] font-semibold text-[var(--on-surface-variant)] mb-4 flex items-center gap-1.5" *ngIf="edital.cargo">
          <span class="material-symbols-outlined !text-[16px] text-[#5d3bf6] shrink-0">badge</span>
          <span>{{ edital.cargo }}</span>
        </p>

        <!-- Progress Bar -->
        <div class="space-y-1.5 mb-4 sm:mb-5 bg-white/70 dark:bg-[#1e293b]/60 p-3 sm:p-3.5 rounded-2xl border border-indigo-100/80 dark:border-[var(--outline-variant)]/60">
          <div class="flex justify-between items-center text-xs sm:text-[13px] font-bold">
            <span class="text-[var(--on-surface)] flex items-center gap-1.5 text-xs sm:text-[13px]">
              <span class="material-symbols-outlined !text-[16px] text-[#5d3bf6]">checklist_rtl</span>
              Progresso do Checklist
            </span>
            <span class="text-[#5d3bf6] font-black text-xs sm:text-[13px]">{{ progressPercentage }}%</span>
          </div>
          <div class="w-full h-2.5 bg-indigo-100/60 dark:bg-slate-700/50 rounded-full overflow-hidden p-0.5">
      ...
      <div class="pt-3.5 border-t border-[var(--outline-variant)]/60 flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-2.5">
        <div class="grid grid-cols-2 sm:flex sm:flex-wrap sm:items-center gap-2 w-full sm:w-auto flex-1">
          <a [routerLink]="['/disciplinas', edital.id]"
             class="bg-[#5d3bf6] hover:bg-[#4c2be8] text-white py-2 px-3.5 rounded-xl text-xs font-extrabold flex items-center justify-center gap-1.5 text-center shadow-sm hover:shadow-md hover:scale-[1.02] active:scale-95 transition-all"
          ...
          <a *ngIf="edital.pareto_data?.pareto_analisado" [routerLink]="['/pareto', edital.id]"
             class="py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 bg-white dark:bg-[#1e293b] text-[#5d3bf6] hover:bg-[#5d3bf6]/10 border border-[#5d3bf6]/30 transition-all text-center"
          ...
          <a [routerLink]="['/sprints', edital.id]"
             class="py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 bg-white dark:bg-[#1e293b] text-[var(--on-surface)] border border-[var(--outline-variant)] hover:border-[#5d3bf6] hover:text-[#5d3bf6] transition-all text-center"
             title="Ver Cronograma de Estudos">
            <span class="material-symbols-outlined !text-[17px] text-[#5d3bf6]">calendar_month</span>
        ...
        <div class="flex items-center justify-end gap-2 w-full sm:w-auto">
          ...
          <ng-container *ngIf="isAdmin">
            <button
              ...
              class="flex-1 sm:flex-initial px-3 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1 bg-amber-500/10 text-amber-700 dark:text-amber-400 hover:bg-amber-500/20 border border-amber-500/20 transition-all cursor-pointer"
            ...
            <button
              ...
              class="flex-1 sm:flex-initial px-3 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer"
              [ngClass]="sendOpenId === edital.id ? 'bg-[#5d3bf6] text-white shadow-sm' : 'bg-[#5d3bf6]/10 text-[#5d3bf6] hover:bg-[#5d3bf6]/20 border border-[#5d3bf6]/20'">
            ...
            <button
              ...
              class="flex-1 sm:flex-initial px-3 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1 bg-rose-500/10 text-rose-600 dark:text-rose-400 hover:bg-rose-500/20 border border-rose-500/20 transition-all cursor-pointer"
            ...
            <ng-container *ngIf="dismissConfirmId === edital.id; else studentDismissBtn">
              <div class="flex gap-1 flex-1 sm:flex-initial justify-end">
                <button
                  ...
                  class="flex-1 sm:flex-initial px-3 py-2 rounded-xl text-xs font-bold bg-[#dc2626] text-white flex items-center justify-center gap-1 cursor-pointer">
                  <span class="material-symbols-outlined !text-[14px]">check</span>
                  Confirmar
                </button>
                <button
                  (click)="cancelDismiss.emit()"
                  class="py-2 px-2.5 rounded-xl text-xs font-bold border border-[var(--outline-variant)] text-[var(--on-surface-variant)] hover:border-[#5d3bf6] hover:text-[#5d3bf6] cursor-pointer">
